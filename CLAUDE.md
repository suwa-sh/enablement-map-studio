...
##### グリッドスナップ
  ...
  - 背景グリッドは10pxグリッドで表示されます。
  - タスクとレーンの位置（position）は、ドラッグ中も含めて常に10pxグリッドにスナップします。
  - レーンのサイズ（width, height）も、リサイズ中も含めて常に10pxグリッドにスナップします。
  - グリッドスナップにより、座標が整数値（10の倍数）で管理され、要素が整然と配置されます。

**実装の構成**:
  - `utils/grid.ts`: グリッドスナップのコアユーティリティ（GRID_SIZE定数とスナップ関数）
  - `utils/positioning.ts`: タスクノードの配置ロジック（TASK_NODE_WIDTH/HEIGHT定数をエクスポート）

**デザイン判断**:
  - 以前のバージョンでは、タスクのドラッグ時に他のタスクと中央が揃う位置にスナップするアライメントガイド機能がありましたが、実装の複雑さと信頼性の観点から削除されました。
  - 現在は、全ての操作（タスクのドラッグ、レーンの移動、レーンのリサイズ）で統一的に10pxグリッドスナップのみを使用しています。
  - これにより、シンプルで予測可能な動作を実現し、保守性を向上させています。

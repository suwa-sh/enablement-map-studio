...
| Multiline TextFields | `multiline`, `minRows` | 複数行入力フィールドは可変高さ（`minRows`）を使用し、内容に応じて自動的に高さが調整される |
| TEXTAREA_ROWS定数 | `{ SMALL: 3, LARGE: 6 }` | 全エディタで統一された行数定数を使用 |
...
#### 4.3.3. プロパティパネル
  ...
  - 画面右側に表示され、選択したレーンやタスクの名前、種別、説明などを編集します。
  - レーンとタスクには、補足情報を記載するための説明フィールド（任意）があります。
  - 説明フィールドは複数行入力が可能で、可変高さ（`minRows={3}`）で表示されます。
  - 要素を削除する際は、確認ダイアログを表示します。
...
#### 4.5.4. プロパティパネル
  ...
  - 画面右側に表示され、選択したEMアクションの名前、説明、および紐づくスキル、ナレッジ、ツールを編集します。
  - EMアクションには、補足情報を記載するための説明フィールド（任意）があります。
  - 説明フィールドは複数行入力が可能で、可変高さ（`minRows={3}`）で表示されます。
...
リソース一覧テーブルに、CSV形式での一括登録機能を提供します。この機能により、既存のリソースをCSVファイルから効率的に登録・更新できます。

##### UI要素

| 要素 | 説明 |
| :--- | :--- |
| CSVダウンロードボタン | 現在のリソース一覧をCSV形式でダウンロード |
| CSVアップロードボタン | CSV形式のファイルをアップロードして一括登録 |

##### CSVフォーマット

CSV形式は、ダウンロード時と同じ9カラム構成です。BOM付きUTF-8エンコーディングで、RFC 4180に準拠した形式をサポートします。

| カラム | 説明 |
| :--- | :--- |
| CSF | 重要成功要因（○または空白、アップロード時は無視） |
| CJMフェーズ | カスタマージャーニーマップのフェーズ名 |
| CJMアクション | カスタマージャーニーマップのアクション名 |
| SBPレーン | サービスブループリントのレーン名 |
| SBPタスク | サービスブループリントのタスク名 |
| 必要な行動 | イネーブルメントマップのアクション名 |
| リソースタイプ | スキル、ナレッジ、ツールのいずれか |
| リソース | リソース名（スキルの場合は「スキル名 / 学習コンテンツタイトル」形式も可） |
| URL | リソースのURL（学習コンテンツのURLなど） |

##### 登録・更新ロジック

アップロード時の処理は、以下のルールに従います。

| 要素種別 | 処理方法 |
| :--- | :--- |
| CJMフェーズ | 名前が一致する場合は既存を参照、一致しない場合は新規作成 |
| CJMアクション | 名前が一致する場合は既存を参照、一致しない場合は新規作成（フェーズに追加） |
| SBPレーン | 名前が一致する場合は既存を参照、一致しない場合は新規作成 |
| SBPタスク | 名前が一致する場合は既存を参照、一致しない場合は新規作成（レーンに追加） |
| EMアクション | SBPタスクを参照する形で新規作成（名前が一致しても別IDで作成） |
| リソース（スキル/ナレッジ/ツール） | 名前が一致する場合は既存を更新、一致しない場合は新規作成 |

**注意事項**:
- CSFカラムの値は無視されます（CSFの設定はOutcomeエディタで行います）
- CJMアクションとSBPタスクの`source_id`連携は考慮しません
- スキルの学習コンテンツは、「スキル名 / 学習コンテンツタイトル」形式で登録できます

##### エラーハンドリング

| エラー種別 | 通知方法 |
| :--- | :--- |
| 空のCSVファイル | トースト通知（エラー） |
| 無効なCSVフォーマット | トースト通知（エラー） |
| 登録成功 | トースト通知（成功） |

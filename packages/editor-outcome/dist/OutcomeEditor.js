...
export function OutcomeEditor() {
    ...
    if (!outcome) {
        return (_jsx(Box, { sx: { display: 'flex', height: '100%', alignItems: 'center', justifyContent: 'center' }, children: _jsx(Typography, { color: "text.secondary", children: "No Outcome data loaded. Please load a YAML file." }) }));
    }
    if (!sbp) {
        return (_jsx(Box, { sx: { display: 'flex', height: '100%', alignItems: 'center', justifyContent: 'center' }, children: _jsx(Typography, { color: "text.secondary", children: "No SBP data loaded. Cannot display Outcome editor." }) }));
    }

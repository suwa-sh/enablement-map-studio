...
export interface AlignmentLine {
  ...
  position: number; // Y座標（horizontal）またはX座標（vertical）
}
...
export function useAlignmentGuides() {
  ...
  const calculateAlignment = useCallback(
    (draggingNode: Node, allNodes: Node[]): AlignmentResult => {
      ...
      const otherNodes = allNodes.filter(
        (node) => node.id !== draggingNode.id && node.type === 'taskNode'
      );
      ...
      otherNodes.forEach((node) => {
        ...
        if (diffY < SNAP_THRESHOLD) {
          // 重複チェック
          const existingHorizontalLine = lines.find(
            (line) => line.type === 'horizontal' && Math.abs(line.position - nodeCenterY) < 1
          );
        ...
        if (diffX < SNAP_THRESHOLD) {
          // 重複チェック
          const existingVerticalLine = lines.find(
            (line) => line.type === 'vertical' && Math.abs(line.position - nodeCenterX) < 1
          );

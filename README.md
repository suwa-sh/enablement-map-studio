...
「組織が求める**成果**」・「成果に必要な**行動**」・「行動に必要な**知識/スキル**」  を一貫して定義できるビジュアルエディタです。
...
実際の組織課題を整理した実例サンプルを [docs/samples/](./docs/samples/README.md) に用意しています。YAML を Open File で開き、自組織用に編集して使えます。

| サンプル | 内容 |
|---|---|
| [AI駆動開発 組織標準化](./docs/samples/ai-dev-org-standardization.md) | AI ツール配布後の個人差を組織標準で埋める導入設計。golden path・guardrail・罠の回避を 1 枚のマップに |
| [全社AI導入 reach→value ギャップ](./docs/samples/enterprise-ai-reach-value.md) | 「配布した(reach)」と「効いている(value)」の距離を診断する導入設計。二軸 KPI・部門別ガードレール・ベンダー出口を 1 枚のマップに |
...
| スキル             | 説明                                                                   | トリガー例                                        |
| ------------------ | ---------------------------------------------------------------------- | ------------------------------------------------- |
| `emstudio-create`  | ヒアリング内容や既存資料からCJM/SBP/Outcome/EMのYAML DSLファイルを生成 | 「イネーブルメントマップを作りたい」「CJMを作成」 |
| `emstudio-analyze` | 既存のYAML DSLファイルの整合性チェック、カバレッジ分析、改善提案       | 「イネーブルメントマップを分析」「DSLのレビュー」 |
| `emstudio-deploy`  | Dockerデプロイやチームでのgit運用フロー構築を支援                      | 「EMスタジオをデプロイ」「チームで運用」          |
...
#### A. イネーブルメントコンテンツリポジトリに配置する場合

特定領域のイネーブルメント用コンテンツをまとめているリポジトリで利用する場合に適しています。チームでスキルを共有できます。

```bash
dest=/path/to/your-enablement-repo/.claude/skills
mkdir -p "$dest"
for d in .claude/skills/emstudio-*; do cp -R "$d" "$dest/$(basename "$d")"; done
```
